...
public class FormValidator {
    ...
    func validateName(title: String , text : String) -> (Bool , String) {
        do {
        let name = try validateText(text, rules: [.empty(name:title), .minimumLength(name: title, len: 3), .maximumLength(name : title , len: 35)])
             return (true,name)
        }catch{
          return (false,"\(error)")
        }
    ...
    func validateStreet(title: String , text : String) -> (Bool , String) {
        do {
            let add = try validateText(text, rules: [.empty(name:title), .minimumLength(name: title, len: 5)])
            return (true,add)
        }catch{
            return (false,"\(error)")
        }
    ...
    func validateZipCode(title: String , text : String) -> (Bool , String) {
        do {
            let add = try validateText(text, rules: [.empty(name:title), .minimumLength(name: title, len: 5), .maximumLength(name: title, len: 5)])
            return (true,add)
        }catch{
            return (false,"\(error)")
        }
    ...
    func validatePassword(title: String = "Password", text : String) -> (Bool , String) {
        do {
            let pwd = try validateText(text, rules: [.empty(name:title), .minimumLength(name: title, len: Constants.minimumLengthPwd) , .maximumLength(name: title, len: Constants.maximumLengthPwd), .isContainsWhitespace(name: title)])
            return (true, pwd)
        }catch{
            return (false,"\(error)")
        }
